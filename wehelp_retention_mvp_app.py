...
def answer_question(question: str, insights: Dict[str, object]) -> str:
    ...
    if any(x in q for x in ["maiores pontos de atenção", "pontos de atenção", "atenção"]):
        ...
        if worst_unit:
            evidence.append(f"a unidade com pior desempenho é **{worst_unit[0]}** com NPS **{worst_unit[1]}**")
        evidence_text = "; ".join(evidence) if evidence else "não há base suficiente para apontar um recorte operacional crítico com segurança"
...
**O que o cliente está sinalizando**
" + "
".join(evidence_parts)
...
**Onde estão os maiores pontos de atenção**
"
            + "
".join(bullets)
            + f"

**Onde a operação está mais pressionada**
{evidence_text}."
            + f"

**Leitura de causa raiz**
As tags de reclamação mais recorrentes são **{top_tags(complaint_tags)}**."
            + comments_text
            + problem_text
            + "

**Conclusão**
O problema principal não parece ser proposta de valor. O que está travando o avanço do NPS é execução operacional nas alavancas que mais influenciam a formação de promotores e detratores."
        )
    ...
    if any(x in q for x in ["melhorar meu nps", "melhorar o nps", "como melhorar"]):
        ...
        for _, row in priorities.head(3).iterrows():
            tp = row["touchpoint"]
            actions.append(
                f"- **{tp}** deve entrar primeiro no plano de ação: ele combina desempenho insuficiente com alto poder de influenciar promotores versus detratores."
            )
            examples = examples_for_touchpoint(tp, "Detrator", 1)
            if examples:
        ...
        focused_actions = []
        if worst_period:
            focused_actions.append(f"- Trate **{worst_period[0]}** como operação prioritária, porque é o período com pior NPS.")
        if worst_unit:
            focused_actions.append(f"- Faça um plano específico para **{worst_unit[0]}**, hoje o recorte com maior pressão operacional.")
        if pd.notna(problems["nps_problem"]) and pd.notna(problems["nps_no_problem"]):
            focused_actions.append(
                f"- Reforce o fechamento de problemas: quem reporta problema tem NPS **{problems['nps_problem']}**, contra **{problems['nps_no_problem']}** entre os demais."
            )
...
**O que fazer para melhorar o NPS**
"
            + "
".join(actions)
            + "

**Onde concentrar a execução**
"
            + ("
...
Não tente melhorar tudo ao mesmo tempo. Escolha 2 ou 3 prioridades, defina dono, prazo e rotina de acompanhamento por unidade, período e plano."
        )
    ...
    if any(x in q for x in ["maior diferencial", "diferencial", "pontos fortes"]):
        ...
        extra_text = "; ".join(extra) if extra else "não há base suficiente para apontar um destaque operacional com segurança"
...
**Qual é o maior diferencial percebido**
"
            + "
".join(bullets)
            + f"
...
**Conclusão**
Seu diferencial competitivo percebido está nos pontos em que a experiência combina boa avaliação com capacidade real de formar promotores."
        )
    ...
    if any(x in q for x in ["3 maiores problemas", "três maiores problemas", "maiores problemas"]):
        ...
        for _, row in priorities.head(3).iterrows():
            tp = row["touchpoint"]
            bullets.append(
                f"- **{tp}** — NPS do ponto **{row['nps_touchpoint']}**, odds ratio **{row['odds_ratio']}**."
            )
            examples = examples_for_touchpoint(tp, "Detrator", 1)
            if examples:
                evidence_parts.append(f"- Em **{tp}**, clientes descrevem situações como: \"{examples[0]}\"")
...
**Os 3 maiores problemas para resolver agora**
"
            + "
".join(bullets)
            + "
...
**Recomendação executiva**
Monte plano de ação com dono, prazo e acompanhamento semanal para esses 3 temas antes de dispersar energia em frentes secundárias."
        )
...
**Hipótese principal**
A queda não parece ser só oscilação estatística. Ela precisa ser investigada a partir dos touchpoints mais críticos e das reclamações mais recorrentes, principalmente **{top_tags(complaint_tags)}**."
                + "
...
Compare o último período com o anterior por unidade, período, tipo de plano e nome do plano para localizar exatamente onde a queda se concentrou."
            )
...
**Perguntas que o MVP já responde melhor**
"
...
def load_file(uploaded_file) -> pd.DataFrame:(uploaded_file) -> pd.DataFrame:(uploaded_file) -> pd.DataFrame:
